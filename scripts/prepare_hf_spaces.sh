...
# Free Gradio Spaces now run only on ZeroGPU, which requires the `spaces`
# package + a @spaces.GPU function (gradio_app.py registers a no-op probe).
# The base image ships `spaces`, but we pin it here to be explicit.
spaces
